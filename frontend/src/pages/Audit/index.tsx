      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="rounded-xl p-6 transition-all duration-200 hover:scale-105"
            ...
            <p className="text-2xl font-bold" style={{ color: 'var(--nexus-text)' }}>{filteredLogs.length.toLocaleString('pt-BR')}</p>
          </div>
          ...
          <div className="rounded-xl p-6 transition-all duration-200 hover:scale-105"
            ...
            <p className="text-2xl font-bold" style={{ color: 'var(--nexus-text)' }}>{userStats.length}</p>
          </div>
          ...
          <div className="rounded-xl p-6 transition-all duration-200 hover:scale-105"
            ...
            <p className="text-2xl font-bold" style={{ color: 'var(--nexus-text)' }}>{criticalActions.length}</p>
          </div>
          ...
          <div className="rounded-xl p-6 transition-all duration-200 hover:scale-105"
            ...
            <p className="text-2xl font-bold" style={{ color: 'var(--nexus-text)' }}>
              {logs.length > 0 ? new Date(logs[0].created_at).toLocaleDateString('pt-BR') : '-'}
            </p>
            <p className="text-xs mt-1" style={{ color: 'var(--nexus-muted-2)' }}>
              {logs.length > 0 ? new Date(logs[0].created_at).toLocaleTimeString('pt-BR') : ''}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="rounded-xl p-6"
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={actionStats} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="space-y-3">
              {userStats.map((user, index) => (
                <div key={index} className="flex items-center justify-between p-3 rounded-lg"
                  ...
                  <span className="text-sm font-semibold" style={{ color: 'var(--nexus-gold)' }}>{user.value}</span>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-xl p-6"
            ...
            <div className="space-y-3">
              {criticalActions.map((log, index) => (
                <div key={index} className="flex items-start gap-3 p-3 rounded-lg"
                  style={{ background: 'rgba(239, 68, 68, 0.05)' }}>
                  <ExclamationTriangleIcon className="w-5 h-5 mt-0.5" style={{ color: '#EF4444' }} />
                  <div className="flex-1">
                    <p className="text-sm font-medium" style={{ color: 'var(--nexus-text)' }}>{actionLabel(log.action)}</p>
                    <p className="text-xs" style={{ color: 'var(--nexus-muted-2)' }}>{new Date(log.created_at).toLocaleString('pt-BR')}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-xl p-6"
            style={{ background: 'var(--nexus-card)', border: '1px solid rgba(var(--nexus-gold-rgb), 0.2)' }}>
            <h3 className="text-lg font-semibold mb-4" style={{ color: 'var(--nexus-text)' }}>Resumo</h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm" style={{ color: 'var(--nexus-muted-2)' }}>Total de registros</span>
                <span className="text-sm font-semibold" style={{ color: 'var(--nexus-text)' }}>{filteredLogs.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm" style={{ color: 'var(--nexus-muted-2)' }}>Módulos ativos</span>
                <span className="text-sm font-semibold" style={{ color: 'var(--nexus-text)' }}>{moduleStats.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm" style={{ color: 'var(--nexus-muted-2)' }}>Tipos de ação</span>
                <span className="text-sm font-semibold" style={{ color: 'var(--nexus-text)' }}>{actionStats.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm" style={{ color: 'var(--nexus-muted-2)' }}>Usuários únicos</span>
                <span className="text-sm font-semibold" style={{ color: 'var(--nexus-text)' }}>{userStats.length}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl p-6 mb-8"
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={8} className="text-center py-8" style={{ color: 'var(--nexus-muted-2)' }}>
                      Carregando...
                    </td>
                  </tr>
                  ...
                  filteredLogs.map((log) => (
                    <tr key={log.id} className="transition-all duration-200 hover:bg-opacity-50"
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium" style={{ color: 'var(--nexus-text)' }}>{log.user_name}</p>
                            <p className="text-xs" style={{ color: 'var(--nexus-muted-2)' }}>Administrador</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-4">
          ...
          <div className="flex items-center justify-between mt-6">
            <p className="text-sm" style={{ color: 'var(--nexus-muted-2)' }}>
              Mostrando {filteredLogs.length} de {filteredLogs.length} registros
            </p>
