    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-brand-graphiteWine/60 mt-1">Registro de vendas</p>
        </div>
        <button onClick={() => { setShowModal(true); setError(''); }} className="btn-primary">Nova Venda</button>
      </div>
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          ...
          <div className="p-8 text-center text-brand-graphiteWine/70">Nenhuma venda encontrada</div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {sales.map((s) => (
                  <tr key={s.id} className="hover:bg-[rgba(214,179,112,0.18)]">
                    <td className="py-3 px-4 text-brand-graphiteWine/70">{new Date(s.created_at).toLocaleDateString('pt-BR')}</td>
                    ...
                    <td className="py-3 px-4 text-brand-graphiteWine/70">{s.notes || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" className="btn-primary">Concluir Venda</button>
              </div>
            </form>
          </div>
        </div>
